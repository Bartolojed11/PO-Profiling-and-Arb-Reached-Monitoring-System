...
if (isset($_POST['submit'])) {
    $form_image = isset($_POST['arbo_form_image'])? $_POST['arbo_form_image'] : '';
    ...
    $oranization_type = isset($_POST['oranization_type']) ? $_POST['oranization_type'] : '';
    $affliation = isset($_POST['affliation']) ? $_POST['affliation'] : '';
    $par1_name_org_assisting = isset($_POST['par1_name_org_assisting']) ? $_POST['par1_name_org_assisting'] : '';
    ...
    $total_arb = isset($_POST['part3_arb1_members']) ? $_POST['part3_arb1_members'] : 0;
    ...
    $total_non_arb = isset($_POST['part3_arb2_members']) ? $_POST['part3_arb2_members'] : 0;
    ...
    $total_members = isset($_POST['part3_total_members']) ? $_POST['part3_total_members'] : 0;
    $total_hh_arb = isset($_POST['total_hh_arb']) ? $_POST['total_hh_arb'] : 0 ;
    $male_hh_arb = isset($_POST['male_hh_arb']) ? $_POST['male_hh_arb'] : 0 ;
    $female_hh_arb = isset($_POST['female_hh_arb']) ? $_POST['female_hh_arb'] : 0 ;

    $capital_amount = isset($_POST['part3_capital_amount']) ? $_POST['part3_capital_amount'] : 0;
    $capital_savers = isset($_POST['part3_capital_savers']) ? $_POST['part3_capital_savers'] : 0;
    $savings_amount = isset($_POST['part3_savings_amount']) ? $_POST['part3_savings_amount'] : 0;
    $savings_savers = isset($_POST['part3_savings_savers']) ? $_POST['part3_savings_savers'] : 0;
    $total_amount = isset($_POST['part3_total_amount']) ? $_POST['part3_total_amount'] : 0;
    $total_savers = isset($_POST['part3_total_savers']) ? $_POST['part3_total_savers'] : 0;
    $liability_amount = isset($_POST['part3_liability_amount']) ? $_POST['part3_liability_amount'] : 0;
    $liability_savers = isset($_POST['part3_liability_savers']) ? $_POST['part3_liability_savers'] : 0;
	$networth_amount = isset($_POST['part3_networth_amount']) ? $_POST['part3_networth_amount'] : 0;
    $networth_savers = isset($_POST['part3_networth_savers']) ? $_POST['part3_networth_savers'] : 0;
    ...
    $assoc_firstname = $_POST['assoc_firstname'];
    ...
    $assoc_cloa_number = isset($_POST['assoc_cloa_number']) ? $_POST['assoc_cloa_number'] : array();
    ...
    $arb_stat = 1;
    $msc = microtime(true);
    $sql_po_profile = 'INSERT INTO arbo_profile (as_of,registration_form_image, NAME, ACRONYM, ARBO_PROVINCE , ARBO_CITY, ARBO_BRGY, ARBO_ADDR_OTHERS,
    ...
    date_registered, registration_no, agency_registered, type_of_organization, AFFILIATION,date_created)
    values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    $run_sql_po_profile->bind_param('ssssssssssssssssssss',
                    $as_of_date,$image_name , $arbo_name, $acro_name, $prov, $city, $brgy, $others, $ope_prov,
                    $ope_city, $ope_brgy, $ope_others, $contact_person, $date_organized, $date_reg,  $reg_num,
                    $agency_registered, $oranization_type, $affliation, $date_created);

    if ($run_sql_po_profile->execute() == true) {
        $prof_id = getProfId($arbo_name);
        $i = 0;
        $j = 0;
        $sql_tbl_ngo_org ='INSERT arbo_ngo_or_org_assist (name_of_ngo_assisting,ngo_year_assisted,arbo_profile_id) VALUES (?,?,?)' ;
        $run_sql_tbl_ngo_org = $conn->prepare($sql_tbl_ngo_org);
        for ($i = 0; $i < count($ngo_organization_assisting); $i++) {
            if(isset($ngo_organization_assisting[$i]) && !empty($ngo_organization_assisting[$i])) {
        ...
        (ARBO_PROFILE_ID, TOTAL_ARB_MALE, TOTAL_ARB_FEMALE, TOTAL_NON_ARB_MALE, TOTAL_NON_ARB_FEMALE, male_hh_arb, female_hh_arb)
            values(?,?,?,?,?,?,?)';
        $run_sql_arbo_membership = $conn->prepare($sql_arbo_membership);
        $run_sql_arbo_membership->bind_param('iiiiiii', $prof_id, $total_non_arb_male,  $total_non_arb_female, $total_arb_male, $total_arb_female,
                 $male_hh_arb, $female_hh_arb);
        $run_sql_arbo_membership->execute();

        $fin_stat_type_id = array(1, 2, 3, 4, 5);

        $sql_fin_stat = 'INSERT INTO arbo_financial_status(arbo_financial_type_id,arbo_profile_id,amount,no_of_savers) values(?,?,?,?)';
        $run_sql_fin_stat = $conn->prepare($sql_fin_stat);

        $run_sql_fin_stat->bind_param('iisi', $fin_stat_type_id[0], $prof_id, $capital_amount, $capital_savers);
        $run_sql_fin_stat->execute();

        $run_sql_fin_stat->bind_param('iisi', $fin_stat_type_id[1], $prof_id, $savings_amount, $savings_savers);
        $run_sql_fin_stat->execute();

        $run_sql_fin_stat->bind_param('iisi', $fin_stat_type_id[2], $prof_id, $total_amount, $total_savers);
        $run_sql_fin_stat->execute();

        $run_sql_fin_stat->bind_param('iisi', $fin_stat_type_id[3], $prof_id, $liability_amount, $liability_savers);
        $run_sql_fin_stat->execute();
		
		$run_sql_fin_stat->bind_param('iisi', $fin_stat_type_id[4], $prof_id, $networth_amount, $networth_savers);
        $run_sql_fin_stat->execute();
        ...
        for ($i = 0; $i < count($purpose_of_loan); $i++) {
            if(!empty($purpose_of_loan[$i])) {
                ...
                $loan_flag = $i;
            } else if(empty($purpose_of_loan[$i]) && $i > 0) {
                ...
                // $run_sql_loans_availed->bind_param('isssssss', $prof_id, $purpose_of_loan[$i-1], $loan_amount[$i], $loan_source[$i], $loan_date_released[$i], 
        //     $loan_date_availed[$i], $loan_terms_payment[$i], $loan_amount_paid[$i]);
        ...
        for ($i = 0; $i < count($train_title); $i++) {
            ...
            if(isset($train_title[$i]) && !empty($train_title[$i])) {
        ...
        for ($i = 0; $i < count($off_bod_fname); $i++) {
            if(isset($off_bod_fname[$i]) || isset($off_bod_lname[$i]) || isset($off_bod_mname[$i])) {
        ...
        for($i = 0; $i < count($committee_type); $i++) {
            for($j = 0 ; $j < count($_POST['com_mem_fname'.$com_where[$i]]) ; $j++) {
                $comm_mem_mname = !empty($_POST['com_mem_mname'.$com_where[$i]]) ? $_POST['com_mem_mname'.$com_where[$i]] : ' ';
                $comm_mem_lname = !empty($_POST['com_mem_lname'.$com_where[$i]]) ? $_POST['com_mem_lname'.$com_where[$i]] : ' ';
                $comm_mem_fname = !empty($_POST['com_mem_fname'.$com_where[$i]]) ? $_POST['com_mem_fname'.$com_where[$i]] : ' ';
    
                $committee_position = $_POST['committee_pos'.$com_where[$i]];
        ...
        foreach($assoc_landsize as &$item) {
            $item = preg_replace("/[^0-9.]/", "", $item);
            // preg_replace("/[^0-9.]/", "", $assoc_landsize[$i]);
        }
        ...
        $run_sql_assoc_member = $conn->prepare($sql_assoc_member);
            ...
            for($i = 0; $i < count($assoc_firstname); $i++) {
                if(isset($assoc_firstname[$i]) && isset($assoc_lastname[$i])) {
                    ...
                    // $assoc_cloa_number[$i] = addIndex($i , '' ,$assoc_cloa_number);
                    ...
                    // $assoc_landsize_ = preg_replace("/[^0-9.]/", "", $assoc_landsize[$i]);
                    ...
                    // var_dump($assoc_mem_id);echo'<br>';
                    // var_dump($assoc_arb_type_);echo'<br>';
                    $run_sql_assoc_member->bind_param('iiisiisssissiiiiiis' ,
                    $prof_id, $assoc_mem_id ,
                    $assoc_position_ , $assoc_gender_ ,
                    $assoc_arb_type[$i] ,
                    $assoc_status_,
                    $assoc_cloa_number[$i],
                    $assoc_landsize[$i],
    ...
    echo '<script>
            alert("Added Successfully!");
                window.location.href="../po_profile_form.php";
        </script>';
        // window.location.href="arbo_form.php";
} //end  if arbo profile is inserted
 else {
    echo '<script>
...
function getProfId($arbo_name) {
    require 'connection.php';
    $sql = $conn->prepare('SELECT id from arbo_profile where name = ?');
    $sql->bind_param('s', $arbo_name);
    $sql->execute();
    $sql->bind_result($id);
    $sql->store_result();
    $sql->num_rows();
